...
def get_namespace_config(ns: str) -> NamespaceConfig:
    ...
    if not config:
        ...
        print(f"Run 'deadrop ns list' to see available namespaces.", file=sys.stderr)
        sys.exit(1)
    return config
...
@ns_app.command
def delete(ns: str, *, force: bool = False, remote: bool = False):
    """Delete a namespace.
...
@identity_app.command
def list(ns: str, *, remote: bool = False):
...
@identity_app.command
def show(ns: str, identity_id: str):
...
@identity_app.command
def export(ns: str, identity_id: str, *, format: str = "text"):
...
@identity_app.command
def delete(ns: str, identity_id: str, *, force: bool = False, remote: bool = False):
    """Delete an identity.
    ...
    if remote:
        if not force:
            confirm = input(f"Delete identity {identity_id} from SERVER and all its messages? [y/N] ")
            if confirm.lower() != "y":
                print("Cancelled.")
                return
    ...
    if identity_id in ns_cfg.mailboxes:
        ...
        ns_cfg.save()
        print(f"Identity removed from local config.")
    elif not remote:
...
def list_batches(ns: str | None = None):
    ...
    for batch in batches:
        print(f"{batch['batch_id']}  ns={batch['ns']}  count={batch['message_count']}  path={batch['archive_path']}")
